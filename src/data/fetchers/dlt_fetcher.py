    ...
    try:
        ...
        for encoding in encodings:
            try:
                ...
                response.text
                logging.info(f"使用 {encoding} 编码解析内容")
                break
        ...
        if not tbody:
            logging.warning("找不到ID为 'tdata' 的表格体")
        ...
        for tr in trs:
            ...
            try:
                tds = tr.find_all("td")
                if len(tds) < 9:  # 大乐透需要至少9列(期数+5红球+2蓝球+其他信息)
                ...
                for i in range(2):
                    if i + 6 < len(tds):
                        blue_ball = tds[i+6].get_text().strip()
                        try:
                            item[f"蓝球_{i+1}"] = int(blue_ball) if blue_ball.isdigit() else 0
                        except ValueError:
                            logging.warning(f"无法解析蓝球_{i+1}: {blue_ball}")
                            item[f"蓝球_{i+1}"] = 0
                    else:
                        logging.warning(f"蓝球_{i+1}不存在")
                        item[f"蓝球_{i+1}"] = 0
                
                # 验证球号是否在有效范围内
                valid_data = True
                for i in range(5):
                    if not (1 <= item[f"红球_{i+1}"] <= 35):
                        logging.warning(f"红球_{i+1} 值无效: {item[f'红球_{i+1}']}")
                        valid_data = False
                        break
                
                for i in range(2):
                    if not (1 <= item[f"蓝球_{i+1}"] <= 12):
                        logging.warning(f"蓝球_{i+1} 值无效: {item[f'蓝球_{i+1}']}")
                        valid_data = False
                        break
...
def fetch_dlt_data():
    """
    获取并保存大乐透历史数据到 'data/dlt/dlt_history.csv'
    """
    name = "dlt"
