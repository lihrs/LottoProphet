...
class CatBoostModel(BaseMLModel):
    ...
    def train(self, df):
        ...
        red_model = self.train_catboost(X_train, red_train_data[0], 'red')
        self.models['red'] = red_model
        
        # 训练蓝球模型
        self.log("训练蓝球CatBoost模型...")
        blue_model = self.train_catboost(X_train, blue_train_data[0], 'blue')
    ...
    def train_catboost(self, X_train, y_train, ball_type):
        ...
        # 训练模型
        try:
            random_search.fit(X_train, y_train, callback=logging_callback)
            ...
            default_model.fit(X_train, y_train, verbose=0)
    ...
    def load_models(self):
        ...
        # 如果所有模型都成功加载，返回True
        if models_loaded:
            self.log(f"CatBoost模型加载成功")
            return True
        elif balls_loaded >= 2:  # 至少加载了红球和蓝球
            # 即使有警告，只要基础模型存在，我们也认为模型可用
            self.log(f"CatBoost模型加载成功，但有一些警告")
            return True
        else:
            self.log(f"CatBoost模型加载失败，未找到必要的红球和蓝球模型")
            return False
